...
def build_dict_from_csv(filename, key_field_index, value_field_index):
    ...
    with open(filename, 'r', newline='') as f:
        ...
        for row in csv_reader: 
            ...
            value_fields = tuple(
                row[i].strip()
                for i in value_field_index
                if i < len(row) 
            )
            #if value_fields is empty
            if not value_fields:
                continue
...
def search_by_date(date):
    #construct a dictionary between:
    #date:measurement, location, geo_code
    ...
    #build a second dictionary between geo_ids and UHF
    output = []
